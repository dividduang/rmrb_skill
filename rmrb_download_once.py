"""
人民日报PDF一次性下载器 - Playwright版本
简化版：去除定时任务，只执行当天一次性下载
"""
...
def download_and_merge_pdfs(date_str, pdf_urls):
    ...
    # 保存合并的PDF
    if success_count > 0:
        merged_filename = os.path.join(download_dir, f"人民日报-{date_str}.pdf")
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            if result:
                ...
                logging.info(f"文件保存位置: 人民日报下载/人民日报-{date_str}-完整版.pdf")
                logging.info("=" * 50)
                return True
            else:
